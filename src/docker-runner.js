 ...
 */
function runStep(repoPath, command, onLog) {
  return new Promise((resolve, reject) => {
    ...
    // node:18-alpine → small Node image, has npm/node available
    const dockerCmd = `docker run --rm -v "${repoPath}:/app" -w /app node:18-alpine sh -c "${command}"`;
